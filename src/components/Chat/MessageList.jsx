...
import { GET_CHAT_MESSAGES, CHAT_MESSAGE_ADDED, CHAT_MESSAGE_UPDATED, CHAT_MESSAGE_DELETED, MARK_CHAT_AS_READ } from '../../graphql/chat';
...
const MessageList = ({ roomId, onReply, onEdit, members }) => {
    ...
    useSubscription(CHAT_MESSAGE_DELETED, {
        ...
        onData: ({ data }) => {
            if (data?.data?.chatMessageDeleted) {
                ...
                setMessages((prev) =>
                    prev.map(m => m.id === deletedInfo.id
    ...
    if (error) {
        return (
            <div className="flex items-center justify-center h-full text-red-500" style={{ padding: '24px' }}>
