      ...
      <Card size="small">
        <Row gutter={[16, 16]}>
          ...
          <Col span={12}>
            ...
            <div>
              <Text strong>
                {dayjs(invoiceData.invoiceDate).format("DD MMMM YYYY")}
              </Text>
            </div>
          </Col>
          <Col span={24}>
      ...
      >
        <Descriptions bordered column={2} size="small">
            ...
            <Tag
              color={
                invoiceData.status === "FULLY_PAID"
                  ? "green"
                  : invoiceData.status === "PARTIAL_PAID"
                    ? "orange"
          ...
          <Descriptions.Item label="Due Date" span={1}>
            {invoiceData.dueDate ? (
              <Text
                type={
                  dayjs(invoiceData.dueDate).isBefore(dayjs())
                    ? "danger"
                    : undefined
                }
              >
            ...
            <Table
                ...
                },
                {
                  ...
                  render: (_, record) => {
                    const finalRate =
                      ...
                      record.customRate !== null
                        ? record.customRate
                        : record.grading?.defaultRate || 0;
                    const amount =
                      parseFloat(record.imageQuantity) * parseFloat(finalRate);
              ...
              summary={(pageData) => {
                const totalAmount = pageData.reduce((sum, item) => {
                  const finalRate =
                    item.customRate !== undefined && item.customRate !== null
                      ? item.customRate
                      : item.grading?.defaultRate || 0;
                  return (
                    sum + parseFloat(item.imageQuantity) * parseFloat(finalRate)
                  );
                }, 0);
                return (
        ...
        >
          <Row gutter={[16, 8]}>
            ...
            <Col span={12} style={{ textAlign: "right" }}>
              <Text strong>
                ₹{parseFloat(invoiceData.subtotalAmount || 0).toLocaleString()}
              </Text>
            </Col>
            {invoiceData.taxAmount > 0 && (
              <>
                ...
                <Col span={12} style={{ textAlign: "right" }}>
                  <Text>
                    ₹{parseFloat(invoiceData.taxAmount).toLocaleString()}
                  </Text>
                </Col>
              </>
            )}
            {invoiceData.discountAmount > 0 && (
              <>
                ...
                <Col span={12} style={{ textAlign: "right" }}>
                  <Text type="danger">
                    -₹{parseFloat(invoiceData.discountAmount).toLocaleString()}
                  </Text>
                </Col>
              </>
            )}
            ...
            <Col span={12} style={{ textAlign: "right" }}>
              <Text
                ...
                style={{
                  fontSize: "18px",
                  color: invoiceData.balanceAmount > 0 ? "#f5222d" : "#52c41a",
                }}
              >
        ...
        {invoiceData.allocations && invoiceData.allocations.length > 0 && (
          <Card
            size="small"
            style={{ marginTop: 16 }}
            title={<Text strong>Payment Allocations</Text>}
          >
            <Table
              size="small"
              bordered
